...
app.get('/api/users', (req, res) => {
  ...
  if (queryId) {
    ...
    if (user) {
      ...
      sendResponse(res, 404, 'User not found with the given ID.')
    }
  } else if (search) {
    ...
    if (filteredUsers.length > 0) {
      ...
      sendResponse(res, 404, '', filteredUsers);
    } else {
      // res.status(400).json({ success: 'false', message: 'No result found' });
      sendResponse(res, 400, 'No result found')
    }
  } else {
...
app.delete('/api/users', (req, res) => {
  ...
  if (userIndex !== -1) {
    ...
    sendResponse(res, 200, 'User deleted successfully', {id: queryId});
  } else {
    // res.status(400).json({ success: 'false', message: 'User doesn\'t exist.', id: queryId });
    sendResponse(res, 200, 'User doest\'t exist.', {id: queryId});
  }
});
